...
@pytest.mark.parametrize("cell", [None, (35.0, 45.0, 99.0, 90.0, 90.0, 90.0)])
@pytest.mark.parametrize("log", [None, "log"])
def test_read_si(IOtest_si, IOtest_log, spacegroup, cell, log):
    """
    Test rs.read_precognition() with a .si file
    """
    # Hacky way to parametrize a pytest fixture
    if log == "log":
        log = IOtest_log
    ...
    if log:
        with pytest.warns(UserWarning):
            result = rs.read_precognition(
                IOtest_si, spacegroup=spacegroup, cell=cell, logfile=log
            )
    else:
        result = rs.read_precognition(
            IOtest_si, spacegroup=spacegroup, cell=cell, logfile=log
